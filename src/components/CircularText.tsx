...
const CircularText: React.FC<CircularTextProps> = ({
  ...
  const width = widthMatch ? `${widthMatch[1]}px` : '200px';
  const height = heightMatch ? `${heightMatch[1]}px` : '200px';

  return (
    <motion.div
      className={`m-0 mx-auto rounded-full relative font-black text-white text-center cursor-pointer origin-center ${className}`}
      style={{ rotate: rotation, width, height, willChange: 'transform', contain: 'layout style paint' }}
      initial={{ rotate: 0 }}
      animate={controls}
